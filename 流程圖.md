...
---
## 3. 流程 #1 初始化 (CO_CANmodule_init) (修正版)
```mermaid
flowchart TD
    A([Start]) --> B[CO_CANmodule_init]
    B --> C[Scan LMOs]
    C --> D{Single MO?}
    D -->|Yes| E[Set singleMoMode; RX=0x000; SW filter]
    D -->|No| F{Multi-RX criteria?}
    F -->|No| G[SW only mode]
    F -->|Yes| H[Set multiRxMode]
    H --> I[Collect RX idx]
    I --> J[Reserve last as wildcard]
    J --> K[Dec usable RX count]
    K --> L[Init binding table]
    L --> Z[Wait for registrations]
```
說明:
1. Single MO? 判斷條件: (RX=1 且 TX=1 且 總 LMO<=2)
2. Multi-RX criteria?: (TX>=1 且 RX>=2)
3. Reserve last as wildcard: 先標記最後一個 RX, 實際接收效果需實測; 失敗可回收。
4. Dec usable RX count: 剩餘可供綁定的硬體 RX 數量 = totalRxLmos - 1 (去掉 wildcard)。
5. 若不符合任一模式 → 退化為全部軟體過濾 (SW only mode)。
備註: 此圖刻意全部使用 ASCII/英文, 並避免 <br/>、中文、括號中的中文，以符合 GitHub 目前 Mermaid parser 容忍度。
